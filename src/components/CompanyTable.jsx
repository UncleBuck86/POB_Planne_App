...
export default function CompanyTable({ rowData: initialRowData = [], dates = [], flightsOut = {}, flightsIn = {}, comments = {} }) {
	// State
	...
	// Undo/Redo logic
	const pushUndo = () => {
		setUndoStack(prev => [...prev, { rowData: JSON.parse(JSON.stringify(rowData)), localComments: { ...localComments } }]);
		setRedoStack([]);
	};

	// Table cell focus helper
	...
	return (
		<div>
			...
			<div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 16 }}>
				<button onClick={handleSave} style={{ fontSize: '1em', padding: '6px 18px', background: autosave ? '#bbb' : '#4caf50', color: '#fff', border: 'none', borderRadius: 4 }} disabled={autosave}>Save</button>
				<label style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
					<input type="checkbox" checked={autosave} onChange={e => setAutosave(e.target.checked)} /> Autosave
				</label>
				<button onClick={openEditor} style={{ fontSize: '0.95em', padding: '4px 12px', background: '#eee', border: '1px solid #bbb', borderRadius: 4 }}>Edit Companies</button>
				<button onClick={pushUndo} style={{ padding: '4px 12px', background: '#e3f2fd', border: '1px solid #bbb', borderRadius: 4 }}>Undo</button>
				<button onClick={() => {
					if (undoStack.length > 0) {
						const last = undoStack[undoStack.length - 1];
						setRedoStack(prev => [...prev, { rowData: JSON.parse(JSON.stringify(rowData)), localComments: { ...localComments } }]);
						setRowData(last.rowData);
						setLocalComments(last.localComments);
						setUndoStack(prev => prev.slice(0, -1));
					}
				}} disabled={undoStack.length === 0} style={{ padding: '4px 12px', background: undoStack.length ? '#e3f2fd' : '#eee', border: '1px solid #bbb', borderRadius: 4 }}>Undo</button>
				<button onClick={() => {
				...
				}} disabled={redoStack.length === 0} style={{ padding: '4px 12px', background: redoStack.length ? '#e3f2fd' : '#eee', border: '1px solid #bbb', borderRadius: 4 }}>Redo</button>
					...
					<div style={{ background: '#fff', padding: 24, borderRadius: 8, maxWidth: 400, margin: '60px auto', boxShadow: '0 2px 12px #333', position: 'relative' }}>
						...
						{editCompanies.map((name, idx) => (
							<div key={idx} style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
								...
								>{pinnedCompanies.includes(name) ? 'Unpin' : 'Pin'}</button>
							</div>
						))}
			...
			<div style={{ overflowX: 'auto' }}>
				<table border="1" cellPadding="6" style={{ width: 'auto', borderCollapse: 'collapse', tableLayout: 'auto' }}>
					...
					<tbody>
							...
							<tr key={row.company || idx} style={{ display: hiddenRows.includes(row.company) ? 'none' : '' }}>
								...
								{dates.map((d, colIdx) => {
									...
									if (manuallyHighlighted) {
										bgColor = '#b3e5fc'; // light blue
									} else if (changed) {
										bgColor = '#ffeeba'; // yellow
									}
									return (
										<td key={d.date} style={bgColor ? { background: bgColor } : {}} onDoubleClick={() => {
											setManualHighlights(prev => ({ ...prev, [cellKey]: !prev[cellKey] }));
										}}>
											<input
												type="number"
												value={currVal}
												min={0}
												style={{ width: '60px', textAlign: 'center', background: bgColor ? bgColor : undefined }}
												ref={el => {
						...
						{/* Total Daily POB row at the bottom */}
						...
						<tr style={{ background: '#f5f5f5', fontWeight: 'bold', height: 'auto' }}>
							...
							{dates.map(d => (
								<td key={d.date} style={{ verticalAlign: 'top', whiteSpace: 'pre-wrap' }}>{flightsOut[d.date]?.join(', ') || ''}</td>
							))}
						</tr>
						{/* Flights In row */}
						<tr style={{ background: '#f5f5f5', fontWeight: 'bold', height: 'auto' }}>
							...
							{dates.map(d => (
								<td key={d.date} style={{ verticalAlign: 'top', whiteSpace: 'pre-wrap' }}>{flightsIn[d.date]?.join(', ') || ''}</td>
							))}
						</tr>
						{/* Editable Comments row */}
									...
									<textarea
										...
										onDoubleClick={() => {
											setManualHighlights(prev => ({
												...prev,
												[`comment-${d.date}`]: !prev[`comment-${d.date}`]
											}));
										}}
										rows={1}
										ref={el => {
						...
						{/* Show hidden rows with a checkbox to unhide */}
						{rowData.map((row, idx) => (
								...
								<tr key={row.company + '-hidden'} style={{ background: '#f9f9f9' }}>
									<td colSpan={dates.length + 1}>
										...
										<input
											type="checkbox"
											checked={true}
											onChange={e => toggleRow(row.company, false)}
