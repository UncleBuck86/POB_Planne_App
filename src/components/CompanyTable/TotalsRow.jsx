...
export default function TotalsRow({ rowData, dates, hiddenRows = [] }) {
  ...
  return (
    <tr style={{ background: theme.surface, color: theme.text, fontWeight: 'bold', position:'sticky', bottom:0, zIndex:4 }}>
      ...
      {dates.map(d => {
        // Sum all company values for this date, skipping hidden rows
        const total = rowData.reduce((sum, row) => hiddenRows.includes(row.id) ? sum : sum + (Number(row[d.date]) || 0), 0);
