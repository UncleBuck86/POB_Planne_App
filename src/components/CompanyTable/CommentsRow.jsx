...
export default function CommentsRow({ dates, comments, lastSavedComments, manualHighlights, setComments, pushUndo }) {
  ...
  return (
    <tr style={{ background: theme.surface, color: theme.text, fontStyle: 'italic', height: 'auto' }}>
      ...
      {dates.map(d => {
        const highlight = !!manualHighlights[`comment-${d.date}`];
        const bg = highlight
          ? '#b3e5fc'
          : isDark
            ? theme.surface
            : comments[d.date] !== (lastSavedComments[d.date] || '')
              ? '#fffbe6'
              : theme.surface;
        const textColor = (isDark && highlight) ? '#111' : theme.text;
        return (
          <td key={d.date} style={{ verticalAlign: 'top', whiteSpace: 'pre-wrap', background: bg, color: textColor, border: `1px solid ${borderColor}` }}>
            <textarea
